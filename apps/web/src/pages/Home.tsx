...
function SimpleAreaChart() {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full" preserveAspectRatio="none">
      ...
      <path d={pathD} fill="none" stroke="rgb(107, 114, 128)" strokeWidth="1.5" />
    </svg>
  );
}
...
export function Home() {
  const { apps, menus, routes } = useRegistry();
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {statsData.map((stat, index) => (
          <div key={stat.label} className="animate-in stagger-1" style={{ animationDelay: `${index * 0.05}s` }}>
            <StatCard stat={stat} />
          </div>
        ))}
      </div>
        ...
        <div className="flex items-center justify-between border-b border-border px-4">
            ...
            {tabs.map((tab) => (
              ...
              >
                ...
                {tab === "Past Performance" && (
                  <span className="ml-1.5 rounded bg-muted px-1.5 py-0.5 text-xs">3</span>
                )}
                {tab === "Key Personnel" && (
                  <span className="ml-1.5 rounded bg-muted px-1.5 py-0.5 text-xs">2</span>
                )}
                {activeTab === tab && (
          ...
          <div className="flex items-center gap-2 py-2">
            <button className="flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5 text-sm font-medium transition-colors hover:bg-accent">
              <span className="h-4 w-4 border border-current rounded grid place-items-center text-[10px]">☰</span>
              Customize Columns
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                ...
                <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Header</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Section Type</th>
            ...
            <tbody>
              {tableData.map((row) => (
                <tr key={row.id} className="border-b border-border table-row-hover">
                  ...
                  <td className="px-4 py-3 text-sm text-muted-foreground">{row.limit}</td>
